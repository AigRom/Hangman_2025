...
class View(Tk):
    def __init__(self, model):
        super().__init__() #See on Tk (GUI) jaoks
        self.my_table = None
        ...
        self.title('Poomismäng 2025')
        self.center(self, self.__width, self.__height)

        #Kirjastiilid
        self.__big_font =font.Font(family='Courier', size=20, weight='bold' )#kollane taust #import alt+enter
        self.__default = font.Font(family='Verdana', size=12) # Vaikimisi kirjastiil
        self.__default_bold = font.Font(family='Verdana', size=12, weight='bold')

        # kogu vormi stiil
        self.option_add('*Font', self.__default)

        #Loo kolm paneeli põhiaknale
        ...
        #loome rippmenüü
        self.__cmb_category = self.__cmb_combobox()

        #loome sisestuskasti Entry
        self.__char_input = self.create_entry()

        #sidumine hiire ja klaviatuuri tegevusega
    ...
    def create_frames(self):
        ...
        image = Frame(top, width=130, height=130, background='lightsalmon')
        ...
        return top, bottom, image #Tagastamne widinad
    ...
    def create_labels(self):
        #labelid mille omadusi ei saa muuta(värv, stiil jms)
